...
## `backup_claude_history.py` — forward backup + restore (v0.1.0 SHIPPED 2026-06-22; v0.2.0 restore SHIPPED 2026-06-26)
...
- **backup-v0.2.0 — restore half. ✅ SHIPPED 2026-06-26.** `restore --session|--project|--all` copies transcripts from `~/.claude-code-backups/` back into `~/.claude/projects/`. Dry-run by default (`--apply` writes); preserves mtime; **never shrinks a live file** that grew past the backup (`--force` overrides) — copy-on-grow run backwards. Verified end-to-end live (throwaway delete/restore round-trip; `--all` across the whole machine refused exactly the one in-progress session that had grown). The preview is a grouped table by project with titles, subagent fragments folded into a count (`--verbose` expands). **Metadata repair turned out NOT to be needed for the scoped (CLI) case** and was deferred: building it surfaced that `sessions-index.json` is a Claude Desktop artifact, absent for CLI projects, so a restored JSONL+mtime is already complete state — the @BasedGPT orphan risk lives on the Desktop side ([`restore_claude_desktop.py`](restore_claude_desktop.py)). `repair_session_index()` is a documented no-op seam. Rationale in [NOTES.md](NOTES.md) → the two "As built (backup-v0.2.0)" blocks.
...
- **Deferred from v0.2.0 (low priority, edge cases):**
  - **`Caveat:` title prefix.** `restore`/`list` strip the `<ide_opened_file>…</ide_opened_file>` wrapper off fallback titles, but leave a leading `Caveat: The messages below were generated by the user while running a command…` system note in place — one file on this machine, no clean delimiter to cut at, not worth a guess. Revisit if it shows up commonly.
  - **`repair_session_index()` is a no-op seam.** Wire it in only if a real Claude Desktop restore case ever routes through this script (it shouldn't — that's `restore_claude_desktop.py`'s lane). Don't implement pre-emptively. See [NOTES.md](NOTES.md) "what it deliberately does *not* repair."
  - **A `prune` verb, *if ever*.** Backup deliberately copies everything (empty session shells included — they're near-zero bytes and the copy-on-grow rule must stay dumb). If disk ever becomes a real concern, the answer is an explicit user-run `prune`, never silent skipping during `backup`.
