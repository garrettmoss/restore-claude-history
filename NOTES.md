...
The shape of the Desktop recovery tool that falls out of this: **two operations, both gated on `sessions-index.json` if present.** For Mode A: detect JSONLs in `~/.claude/projects/<encoded>/` whose corresponding metadata file is damaged, then either repair the metadata or restore the historical version from a snapshot. For Mode B: detect JSONL filenames in `sessions-index.json` with no live file, then look for them in TM/local snapshots — if found, restore (this is the case our existing script already handles for CLI-stored JSONLs); if not found across all available snapshots, surface as "not recoverable, here's what we know was there" rather than failing silently.
