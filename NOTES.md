...
### As built (backup-v0.2.0): the `restore` verb, and what it deliberately does *not* repair

`restore` reads our own backups under `~/.claude-code-backups/` and copies transcripts back into `~/.claude/projects/`. Selectors: `--session <project>/<uuid>` (the canonical form — it's literally the manifest key) or a bare `<uuid>` (resolved across projects, refusing on the effectively-impossible cross-project collision rather than guessing), `--project <encoded>`, or `--all`. Dry-run by default; `--apply` writes.

**Never shrink a live file — copy-on-grow run backwards.** The one safety rule that carries the design: `restore` refuses any target whose *live* file is larger than the backup. Append-only JSONLs mean "larger == more complete," so a live file that has grown past the backup is a session you kept chatting in since the last backup ran; overwriting it with the older copy would silently drop that tail. `--force` overrides, deliberately, for the rare "I want the older version back" case. This is the same invariant the backup engine enforces (never overwrite a larger backup with a smaller live file) pointed the other way — and it's not theoretical: tested live against the whole machine, the *one* file `restore --all` refused was the in-progress session that had grown since its backup. The guard protected exactly the file a naive `cp` would have damaged.

**mtime is preserved end to end.** Backups already carry the source mtime (load-bearing — see the mtime mechanism below); `restore` uses the same `copy_preserving_mtime`, so a restored chat lands with its real last-activity time, not `now`. A restore that stamped `now` would look freshly-touched and survive *this* cleanup pass, but the chain has to be honest the whole way: the backup holds the true mtime, the restore carries it forward. Verified end-to-end (delete a throwaway transcript, restore it, confirm byte-identical with the original 3-months-old mtime intact).

**Metadata repair is deferred — and the reason is a real finding.** The original roadmap (TODO.md, and the @BasedGPT orphan risk on [#62272](https://github.com/anthropics/claude-code/issues/62272#issuecomment-4554894518)) framed `restore` as needing metadata repair so a restored JSONL isn't re-deleted as an orphan. Building it surfaced that the relevant manifest, `sessions-index.json`, **is a Claude Desktop artifact**: it exists only for projects opened through Desktop's Claude Code area (on this machine, only `data-of-being`), and is *absent* for CLI/VS-Code-extension-driven projects, which function fine without it. So for the case this script is scoped to (CLI transcripts), the JSONL with its preserved mtime *is* the complete state — there's nothing to repair, and fabricating an index the CLI never writes would be inventing state. The orphan concern is genuine but lives on the Desktop side, which is [`restore_claude_desktop.py`](restore_claude_desktop.py)'s job. `repair_session_index()` is left as a documented no-op seam so the call site exists if a Desktop-restore case ever lands here — do **not** wire it in pre-emptively.

**Display ≠ selection (subagents, verbosity).** The `restore` preview folds each chat's subagent fragments into a `(+N subagent(s))` count by default and expands them only under `--verbose`. This is a *display* choice and must never leak into *what gets restored*: `--all` / `--project` always bring the subagent fragments along regardless of verbosity. The data-of-being corpus is the cautionary case — its parent transcripts are the Mode-B losses (deleted before any snapshot), so the subagent fragments are the *only surviving files*; a version that "tidied away" fragments from the restore set would drop the only thing left. Verbose mode surfaces these as an "orphaned subagent(s)" bucket, which doubles as self-documenting data loss.

### As built (backup-v0.2.0): recovering a transcript's display title

`list` and the `restore` preview show each chat by the title you'd recognize from the VS Code / Desktop picker, read straight out of the backed-up JSONL (`read_session_label`). Resolution mirrors the UI's own priority: a manual rename (`custom-title`) wins, else the auto-generated `ai-title`, else the first user prompt. Findings worth keeping:

- **The title is a JSONL record, not separate metadata.** The picker's title is an `ai-title` record (`{"type":"ai-title","aiTitle":"…"}`) Claude appends to the transcript — so it's *in* our backups. It's also bookkeeping churn (it re-appends/regenerates), which is why `ai-title` is in `BOOKKEEPING_RECORD_TYPES`; we read the *last* occurrence, the current value. (`aiTitle` is the only title-bearing field that exists in practice — a full scan of 81 transcripts found no `custom-title`/`title` anywhere; we check for `custom-title` anyway as cheap future-proofing for manual renames.)
- **Read by record *count*, not bytes.** A single record can be enormous — an `<ide_opened_file>` user message can inline a multi-MB image or file. A byte-capped read truncates mid-record and fails to parse the rest, which made big real chats show as untitled. So `read_session_label` iterates whole lines bounded by `LABEL_SCAN_RECORDS`, skipping any one oversized line before parsing it.
- **Strip the IDE prefix.** When a file is open in VS Code, the extension prepends `<ide_opened_file>…</ide_opened_file>` to your message; the real prompt follows the closing tag. We strip that wrapper so the title is your prompt, not "The user opened the file …". The closing tag makes the cut unambiguous; other injected prefixes (a leading `Caveat: …` system note) are left alone — one file, no clean delimiter, not worth guessing.
- **Two honest "no title" states.** `(empty — no messages)` is a *confident* claim: the whole file scanned with no `user`/`assistant` record, only `file-history-snapshot`/bookkeeping — a session shell that was spun up but never chatted in (not a lost chat). `(title not found)` is *honest uncertainty*: there is conversation, we just couldn't extract a title from what we scanned. The old `(untitled)` over-claimed the second case. None of this gates the backup — empty shells are still copied; labeling is display-only.

**Restoration of last resort** is [`restore_claude_code.py`](restore_claude_code.py). When the deletion predates your backups, it assumes the worst has already happened and pulls your chats back out of Time Machine. It's what catches you when both prevention *and* your own backups fall short — which, given the track record, is a "when" not an "if."
