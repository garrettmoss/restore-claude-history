...
The shape of the Desktop recovery tool that falls out of this:

- **Mode A — primary path is surgical edit, not snapshot restore.** For each broken `local_*.json`, find the matching JSONL by `createdAt` ↔ first-record `timestamp` (refuse on ambiguity); write `cliSessionId = <JSONL UUID>`; remove `transcriptUnavailable`; leave every other field alone. No TM drive needed, works on machines with no snapshot coverage at all, smallest blast radius. The snapshot-restore path is the **fallback** for cases where matching is ambiguous (rare in practice but the script must handle it) — restoring the historical metadata file sidesteps the match problem because the snapshot already has the right `cliSessionId` baked in.
- **Mode B — detect via `sessions-index.json`,** restore the missing JSONLs from TM/local snapshots (the case our existing `restore_claude_history.py` already handles for the CLI store), then run the Mode A path on the corresponding metadata. If the JSONLs aren't in any available snapshot, surface as "not recoverable, here's what `sessions-index.json` says was there" rather than failing silently.
- **Preflight for either mode:** Desktop must be quit before mutating metadata (see gotcha above). Refuse to act otherwise.
