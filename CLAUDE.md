...
- **`backup_claude_history.py`** — *planned, not built.* Forward backup of `~/.claude/projects/` (a `SessionStart` hook) plus restore-from-your-own-backups, so recovery stops depending on Time Machine. Architecture locked; see TODO.md (roadmap) and NOTES.md (rationale).
